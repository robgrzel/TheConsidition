...
template <typename T>
T hash(T *k, ub8 length, ub8 level) {
...
--------------------------------------------------------------------
*/
template<typename ub_t>
ub8 hash2(ub_t *k, ub8 length, ub8 level) {
...
*/
template<typename ub_t>
ub8 hash3(ub_t *k, ub8 length, ub8 level) {
	//register ub1 *k;        /* the key */
	//register ub8 length;   /* the length of the key */
	//register ub8 level;    /* the previous hash, or an arbitrary value */
...
template<unsigned N, unsigned n, typename T>
ub8 hash_chunking(T key[N]) {
	...
	for (int i = 0; i < (N / n); i++) {
		printf("i=%d,h=%zu\n",i,h);
		h = hash( key + (i*n), sizeof(int) * n, h);
	}
	
	int m = N - n*(N/n);
	printf("m=%d\n",m);
	
	if (m) h = hash(key + n*(N/n), sizeof(int) * m, h);
...
#endif //OPENCVTEST_FAST_HASH_HPP
